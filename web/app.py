"""AI 面试官 — Gradio Web UI（Phase 4: 替换 Streamlit 为 Gradio）
...
def on_start(jd_file, resume_file):
    ...
    if jd_file is None or resume_file is None:
        return None, None, None, "请先上传 JD 和简历文件", gr.update(visible=True), gr.update(visible=False), gr.update(visible=False)
    ...
    try:
        ...
        return state, info_text, chat, "", gr.update(visible=False), gr.update(visible=True), gr.update(visible=False)

    except Exception as e:
        return None, None, None, f"初始化失败: {e}", gr.update(visible=True), gr.update(visible=False), gr.update(visible=False)
...
def on_submit(answer, state):
    """提交回答 → 评判 → 下一题 或 结束"""
    if state is None:
        return state, [("系统", "请先开始面试")], "", "", gr.update(visible=True), gr.update(visible=False)
    ...
    for r in state["rounds"]:
        q_text = r["question"].content if hasattr(r["question"], "content") else r["question"]["content"]
        a_text = r.get("answer", "")
        j = r.get("judge")
    ...
    if state.get("terminated"):
        ...
        if state.get("terminated"):
            ...
            report_text = _render_report_md(state)
            return state, chat, "", report_text, gr.update(visible=False), gr.update(visible=True)
        next_q = state["question"].content if state["question"] else "面试结束"
        chat.append(("🤖 面试官", next_q))
        return state, chat, "", "", gr.update(visible=True), gr.update(visible=False)
    ...
    with gr.Blocks(title="AI 面试官") as demo:
        ...
        def _end_interview(state):
            ...
            for r in state.get("rounds", []):
                q = r["question"]
                chat.append(("🤖 面试官", q.content if hasattr(q, "content") else q.get("content", "")))
                chat.append(("🧑‍💻 你", r.get("answer", "")))
                j = r.get("judge")
                if j:
                    score = j.score if hasattr(j, "score") else j.get("score", 0)
                    comment = j.comment if hasattr(j, "comment") else j.get("comment", "")
                    chat.append(("📊 评分", f"{score}/100 — {comment}"))
            report_text = _render_report_md(state)
            return state, chat, "", report_text, gr.update(visible=False), gr.update(visible=True)
