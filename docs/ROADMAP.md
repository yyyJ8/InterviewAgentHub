...
| 阶段 | 时间 | 目标 | 状态 |
|------|------|------|------|
| **Phase 1** | 第 1-2 周 | MVP 跑通核心链路 | ✅ 完成 |
| **Phase 2** | 第 3-4 周 | 多轮面试流程 | ✅ 完成 |
| **Phase 3** | 第 5-6 周 | MCP Gateway + 长期记忆 | ✅ 完成 |
| **Phase 4** | 第 7-8 周 | 工程化 + Demo 准备 | 🔄 进行中 |

### Phase 4 变更说明

- **前端从 Streamlit 迁移到 Gradio**：Streamlit 不熟悉，Gradio 有使用经验，且可直接挂载到 FastAPI
- 启动方式统合：`python main.py web` 启动 FastAPI Gateway + Gradio UI（单进程，端口 8000）
...
### 4.0 前端迁移：Streamlit → Gradio ✅

| 任务 | 详情 | 状态 |
|------|------|------|
| 4.0.1 Gradio Web UI | `web/app.py` 重写为 Gradio Blocks（~280 行） | ✅ |
| 4.0.2 挂载到 FastAPI | `gr.mount_gradio_app(app, demo, path="/ui")` | ✅ |
| 4.0.3 统合启动入口 | `main.py web` 启动 Gateway + Gradio UI（单进程 8000 端口） | ✅ |
| 4.0.4 清理 Streamlit | 移除 streamlit 依赖，替换为 gradio | ✅ |

**原因**：不熟悉 Streamlit，Gradio 有使用经验，代码量从 740 行降至 280 行。
...
| 4.1.4 空回答检测 | 连续 3 轮空回答 → 自动结束面试 | 检测逻辑 | ✅ 已有 |
| 4.1.5 ChromaDB 降级 | 连接失败 → 纯内存模式，不影响核心流程 | 降级 | ✅ 已有 |
...
| 任务 | 详情 | 产出 | 状态 |
|------|------|------|------|
| 4.2.1 LLM 流式输出 | 题目生成逐字推送到前端 | SSE/Generator | ⬜ |
| 4.2.2 面试进度指示 | 当前轮次 / 总轮次 / 已覆盖维度展示 | UI 优化 | ⬜ |
| 4.2.3 打字机效果 | Gradio 原生支持或自定义 generator | 前端优化 | ⬜ |
...
> 🔄 **进行中**：Gradio 前端迁移完成，剩余流式输出 + Docker 部署 + Demo 准备  
...
```
Phase 1 ─── ✅ MVP 可用（单轮面试）
Phase 2 ─── ✅ 完整面试闭环（多轮 + 报告）
Phase 3 ─── ✅ 生产级架构（Gateway + 记忆 + Gradio UI）
Phase 4 ─── 🔄 进行中（流式 + Docker + Demo）
```
...
1.2 文件解析 ─┬─→ 1.3 JD Agent ──→ 1.5 匹配 ──→ 1.6 单轮问答 ──→ 1.7 Web UI
              ...
              └─→ 1.4 简历 Agent ──→ 1.5 ────────────────────────────┘
                                                     ...
                                                     2.3 题库 Server ─┤
                                                              ...
                                                              3.1 MCP Gateway
                                                                      │
                                            3.2 ChromaDB + SessionStore ──→ 3.3 API
                                                                      │
                                                                      ↓
                                                        4.0 Gradio UI 迁移 ✅
                                                                      │
                                                              4.2 流式 → 4.3 Docker → 4.4 Demo
```
